...
import DoorLoadingAnimation from "../LoadingSpinner.jsx";
...
const PortfolioLayout = () => {
  ...
  const sections = {
    home: {
      ...
      title: "About Me",
      icon: (
        <FiUser size={20} className="text-gray-600 group-hover:text-pink-600" />
      ),
    },
    portfolio: {
      path: "/portfolio",
    ...
    contact: {
      ...
      title: "Contact Me",
      icon: (
        <FiMail size={20} className="text-gray-600 group-hover:text-pink-600" />
      ),
    },
  };
  ...
  const activeSection = Object.keys(sections).find(
    (key) => location.pathname === sections[key].path,
  );
  ...
  return (
    <>
      <AnimatePresence>{isLoading && <DoorLoadingAnimation />}</AnimatePresence>
      <div className="flex h-screen bg-white overflow-hidden font-sans">
        ...
        {/* Main Content */}
        <div
          className={`flex-1 overflow-auto transition-all duration-300 ${sidebarOpen ? "ml-0" : "ml-0"}`}
        >
          <Outlet />
        </div>
      </div>
